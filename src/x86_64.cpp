...
void X86_64::emit_rex(Reg reg, Reg rm) {
    uint8_t rex = 0x40;
    if (reg.size == OpSize::B64) rex |= 0x08;
    if (reg.is_ext()) rex |= 0x04;
    if (rm.is_ext()) rex |= 0x01;

    bool need_rex = (rex != 0x40) || (reg.size == OpSize::B64);
    bool need_for_low_byte = (reg.size == OpSize::B8 && !reg.high_byte &&
                               reg.num() >= 4 && reg.num() <= 7 && !reg.is_ext());
...
void X86_64::emit_rex(Reg rm) {
    uint8_t rex = 0x40;
    if (rm.size == OpSize::B64) rex |= 0x08;
    if (rm.is_ext()) rex |= 0x01;

    bool need_rex = (rex != 0x40) || (rm.size == OpSize::B64);
    bool need_for_low_byte = (rm.size == OpSize::B8 && !rm.high_byte &&
                               rm.num() >= 4 && rm.num() <= 7 && !rm.is_ext());
...
void X86_64::mov(Reg dst, Reg src) {
    if (dst.size == OpSize::B16) emit8(0x66);
    emit_rex(src, dst);
...
void X86_64::mov(Mem dst, Reg src) {
    if (src.size == OpSize::B16) emit8(0x66);
...
void X86_64::test(Reg a, Reg b) {
    if (a.size == OpSize::B16) emit8(0x66);
    emit_rex(b, a);
...
void X86_64::movss(XMM dst, Mem src) {
    emit8(0xF3);
    uint8_t rex = 0x40;
    if (xmm_ext(dst)) rex |= 0x04;
    if (src.base.is_ext()) rex |= 0x01;
    if (rex != 0x40) emit8(rex);
    emit8(0x0F);
    emit8(0x10);
...
void X86_64::movss(Mem dst, XMM src) {
    emit8(0xF3);
    uint8_t rex = 0x40;
    if (xmm_ext(src)) rex |= 0x04;
    if (dst.base.is_ext()) rex |= 0x01;
    if (rex != 0x40) emit8(rex);
    emit8(0x0F);
    emit8(0x11);
...
void X86_64::cvtsi2ss(XMM xmm, Reg r) {
    emit8(0xF3);
    uint8_t rex = 0x40;
    if (r.size == OpSize::B64) rex |= 0x08;
    if (xmm_ext(xmm)) rex |= 0x04;
    if (r.is_ext()) rex |= 0x01;
    if (rex != 0x40) emit8(rex);
    emit8(0x0F);
    emit8(0x2A);
    emit_modrm(0b11, xmm_num(xmm), r.num());
...
void X86_64::cvttss2si(Reg r, XMM xmm) {
    ...
    uint8_t rex = 0x40;
    if (r.size == OpSize::B64) rex |= 0x08;
    if (r.is_ext()) rex |= 0x04;
    if (xmm_ext(xmm)) rex |= 0x01;
    if (rex != 0x40) emit8(rex);
    emit8(0x0F);
    emit8(0x2C);
